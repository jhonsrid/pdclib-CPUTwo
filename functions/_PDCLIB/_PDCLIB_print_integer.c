...
{
    if ( status->current == 0 && quot == 0 && rem == 0 && status->prec == 0 )
    {
        intformat( 0, status );
    }
    else
    {
        ...
        if ( quot != 0 )
        {
            _PDCLIB_print_integer( quot / status->base, quot % status->base, status );
        }
        else
        {
            intformat( rem, status );
        }
        ...
        {
            PUT( _PDCLIB_digits[ rem ] );
        }
        else
        {
            PUT( _PDCLIB_Xdigits[ rem ] );
        }
    }
}
